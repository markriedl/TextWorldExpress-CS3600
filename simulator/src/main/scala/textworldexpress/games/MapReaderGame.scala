...
class MapReaderGame(val locations:Array[Room], val taskObjects:ArrayBuffer[FastObject], val mapbook:Mapbook, val box:Box, val startLocation:Room, val endLocation:Room, val actualDistanceApart:Int, val limitInventorySize:Boolean, val seed:Long = 0, val generationProperties:Map[String, Int]) extends TextGame {
  ...
  def deepCopy():MapReaderGame = {
    ...
    val game = new MapReaderGame(locationsClone, clonedTaskObjects, clonedMapbook, clonedBox, clonedStartLocation, clonedEndLocation, this.actualDistanceApart, this.limitInventorySize, this.seed, this.generationProperties)
...
object MapReaderGame {
  // Recursively search a set of cloned objects (and their contents) for one with a given name.
  // Used by deepCopy() to relocate the mapbook to its true container, which could be the agent's
  // inventory, a room, or (since the mapbook is movable) some container within a room.
  def findContainerClone(roots:Seq[FastObject], targetName:String):Option[FastObject] = {
    for (obj <- roots) {
      if (obj.name == targetName) return Some(obj)
      val found = findContainerClone(obj.contents.toSeq, targetName)
